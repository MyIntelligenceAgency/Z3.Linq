...
.PARAMETER Version
    The Microsoft.Z3 version to install. Must match the PackageReference version in
    solutions/Z3.Linq/Z3.Linq.csproj.
...
.EXAMPLE
    ...
    Populates the feed ready for `dotnet build` or `dotnet restore`. The InvokeBuild
    pipeline (build.ps1) runs this automatically via the PreBuild task; you only need
    it by hand when building the solution directly (for example from an IDE) on a
    clean clone.

.NOTES
    To move to a new Z3 version, update Version and ExpectedHash here and the matching
    PackageReference version in solutions/Z3.Linq/Z3.Linq.csproj together. The hash of
    a release asset can be obtained with:
